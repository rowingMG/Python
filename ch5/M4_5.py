...
def calculateMinPay(balance):
	...
	abs_minimum = 20000
	pay_rate = float(input('최소상환금액의 비율을 입력하세요: '))
...
def getSummary():
	...
	response = input('요약해서 보시겠습니까? (y/n): ')
	while valid_response:
		if response in ('y', 'Y'):
			return True
		elif response in ('n', 'N'):
...
def calculatePayoff(monthly_pay, balance, interest_rate):
	...
	if not is_summary:
		...
		print('총', str(payment_num) + '회 납입,', format(interest_paid, '.0f'), '원의 이자를 지불')
